# -*- coding: utf-8 -*-
"""bilingual_transliteration(transformer_final).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1O5yZROUU5gI-A3R1_2R6X23uc0HWGG_z
...
df_hindi_trn = getdata('https://drive.google.com/open?id=1sTCskB7TMC5V6tnygw9GhYCvF7tggnqO')
df_hindi_dev = getdata('https://drive.google.com/open?id=1r6oDvbBD16mcbpolSJi5uZ6bsxdz-moT')
...
for d,d2,s in zip(dataf,dataf_test,special_words):
...
def mergeinput(input_texts,target_texts):
  ...
  for input_text,target_text in zip(input_texts,target_texts):
    ...
    for i in range(len(input_text)):
      if(len(input_text[i]) <= 25 and len(target_text[i]) <= 30):
...
input_tensor_train , target_tensor_train = shuffle(input_tensor_train,target_tensor_train)
...
pt_batch, en_batch = next(iter(dataset))
pt_batch, en_batch
...
# @tf.function
def predictAccuracy(mismatch_pair,strmismatch):
  ip = "";
  cnt = 0;
  output = "";
  trueVal = "";
  ...
  for i in range(0, l):
    ...
    for k in result:
      if (idx2charHi[k.numpy()] not in special_words):
        output += idx2charHi[k.numpy()]
    # print(output)
    
    # print(word)
    for r in target_word:
      if(idx2charHi[r] not in special_words):
...
mismatch_pair = [];
strmismatch = "";
a,b,c,d = predictAccuracy(mismatch_pair,strmismatch)
